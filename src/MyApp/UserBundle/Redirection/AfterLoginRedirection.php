...
class AfterLoginRedirection implements AuthenticationSuccessHandlerInterface {
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token) {
        ...
        if (in_array('ROLE_ADMIN', $rolesTab, true) || in_array('ROLE_SUPER_ADMIN', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('admin/dashboard'));
        // sinon, s'il s'agit d'un commercial on le redirige vers le CRM
        elseif (in_array('ROLE_CANDIDAT', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('homeCan'));
        elseif (in_array('ROLE_MONITEUR', $rolesTab, true)) {
            $redirection = new RedirectResponse($this->router->generate('homeMon'));
        }
        // sinon il s'agit d'un membre
        else
            $redirection = new RedirectResponse($this->router->generate('homeRes'));
